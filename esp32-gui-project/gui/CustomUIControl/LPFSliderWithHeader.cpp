...
namespace custom
{
	...
	//
	//-----------------------------------------------------------------*/
	LPFSliderWithHeader::LPFSliderWithHeader(uint16_t groupY, uint16_t y, int maxValue, int value,
		const IUIContext & context, const Action<void(int)> & valueChanged,
		const std::string_view & header, IGElement * gelement)
		:
		LPFSlider(COLUMN1_X+TEXT_WIDTH+MARGIN, y+(ROW_HEIGHT-HPFSLIDER_HEIGHT-LINE_HEIGHT)/2,
	...
	//
	//-----------------------------------------------------------------*/
	LPFSliderWithHeader::~LPFSliderWithHeader()
	{
	...
	//
	//-----------------------------------------------------------------*/
	void LPFSliderWithHeader::SetEnable(bool ena)
	{
		if(ena)
		{
